# CopyM8 ♾️
[Visit the official website](https://copym8.com)
...
- **Strict Reorder Mode**: Enter dedicated Reorder Mode to seamlessly reorder items (or whole folders!) using `Cmd + Up/Down`. Reordering intelligently scopes to your active search filters or specific folder contents. You can also instantly configure metadata via dedicated shortcut fields: `Control + F` to set Freeze limits (Pinned/Groups tabs) and `Control + P` to move the Playhead (Queue tab). Use `Up/Down Arrow` to change values, and hit `Enter` to instantly save and exit Reorder Mode! To maintain focus and prevent accidental changes during rapid reordering, mouse interactions and peripheral controls (like Tabs, Search, Settings, and Trash) are intentionally locked out. The streamlined footer removes unnecessary buttons (like Cancel) to maximize layout breathing room.
...
- **Rolling Local Backups & State Preservation**: CopyM8 automatically snapshots your exact workspace state (including folders, pins, and queue) into up to 3 local rotating backup slots. Accidentally deleted a huge folder? Seamlessly restore your history directly from Settings with one click. The backup engine elegantly manages its own disk footprint, allowing you to easily adjust limits, gracefully purge old slots, and monitor live Backup Storage Sizes directly from the Statistics panel.
