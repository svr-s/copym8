# CopyM8 Shortcuts & Hotkeys

Here is a comprehensive list of all keyboard shortcuts available in CopyM8.

| Action | Shortcut | Description |
| :--- | :--- | :--- |
| **Open Search** | `Cmd + F` | Immediately focuses the search bar to filter copied text and source applications. |
| **Toggle Layout** | `Cmd + D` | Instantly switches between the "Dense" and "Spaced" UI layouts. |
| **Cycle Colors** | `Cmd + K` | Cycles through all available accent color schemes. |
| **Open Settings** | `Cmd + ,` | Toggles the Settings pane. |
| **Close Window** | `Esc` | Instantly dismisses the CopyM8 window. |
| **Select Down** | `Down Arrow` | Moves selection down the clipboard history list. |
| **Select Up** | `Up Arrow` | Moves selection up the clipboard history list. |
| **Expand Item** | `Right Arrow` | Expands the selected item to show its full multi-line text. |
| **Collapse Item** | `Left Arrow` | Collapses the expanded item. |
| **Paste Item** | `Enter` | Pastes the currently highlighted item into the previous application. |
| **Quick Paste** | `1` - `0` | Instantly pastes the plain text of the corresponding item (1 for 1st item, 0 for 10th item). These numbers dynamically re-index when searching! |
| **Rich Paste** | `Cmd + 1-0` | Pastes the corresponding item with its original Rich Text Formatting preserved. |
| **Rich Paste (Selected)** | `Cmd + Enter` | Pastes the currently highlighted item with Rich Text Formatting preserved. |
| **Toggle Edit Mode** | `Cmd + E` | Toggles Edit Mode to show checkboxes for bulk actions. |
| **Switch Tabs** | `Option + 1-6` | Instantly switches between tabs (All, Pinned, Text, Links, Images, Files). |
| **Toggle Pin** | `P` | Instantly pins/unpins the currently highlighted item (or selected items in Edit Mode). |
| **Delete Item** | `Backspace` | Instantly deletes the currently highlighted item (or selected items in Edit Mode). |
| **Toggle Selection (Edit Mode)** | `Space` | Selects or deselects the currently highlighted item when in Edit Mode. |
| **Select All (Edit Mode)** | `Cmd + A` | Selects or deselects all visible items when in Edit Mode. |
